...
const map = {
     ...
     * @param {{x: int, y: int}} blockPoint Точка препятствия
    ...
    render(snakePointsArray, foodPoint, blockPoint) {
        ...
        // Получаем элемент ячейки препятствия по точке blockPoint.
        const blockCell = this.cells[`x${blockPoint.x}_y${blockPoint.y}`];
        // Отображаем препятствие.
        blockCell.classList.add('block');
        // Добавляем элемент ячейки препятствия в массив занятых точек на карте.
...
/**
 * Объект препятствия.
 * @property {int} x Координата X препятствия.
 * @property {int} y Координата Y препятствия.
 */
const block = {
    x: null,
    y: null,
    ...
    /**
     * Отдает координаты препятствия.
     * @returns {{x: int, y: int}} Координаты препятствия.
     */
    getCoordinates() {
        return {
            x: this.x,
    ...
    /**
     * Устанавливает координаты для препятствия.
     * @param {{x: int, y: int}} point Новая точка с координатами препятствия.
     */
    setCoordinates(point) {
        this.x = point.x;
        this.y = point.y;
    ...
    /**
     * Определяет соответствует ли точка на которой находится препятствие той точке что была передана.
     * @param {{x: int, y: int}} point Точка, для проверки соответствия точке препятствия.
     * @returns {boolean} true, если точки совпали, иначе false.
     */
    isOnPoint(point) {
        return this.x === point.x && this.y === point.y;
 ...
 * @property {block} block Объект препятствия.
...
const game = {
    ...
    reset() {
        ...
        // Ставим епрепятствие на карту в случайную пустую ячейку.
    ...
    canMakeStep() {
        ...
        return !this.snake.isOnPoint(this.snake.getNextStepHeadPoint()) && !this.block.isOnPoint(this.snake.getNextStepHeadPoint());
    },
};
